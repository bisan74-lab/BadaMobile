...
1. **히트맵 굽는 비용** — 스크럽 중에는 배경만 갱신하고 손을 뗀 뒤 핵심영역을
   올리기 / 앞뒤 시각 프리페치 / 핵심영역 텍스처 크기 재검토.
   지표: `perf_baseline_test.dart`의 "시간 스텝 1회당 계산 픽셀 수".
2. **상세 예보 캐시** — 지점·시각 단위로 캐시해 재조회 HTTP를 0건으로.
   지표: 같은 테스트의 "재조회 시 요청 수".
...
4. **첫 진입 로딩** — `AppShell`이 `IndexedStack`으로 세 탭을 모두 살려 둬
   전환 자체는 싸다. 처음 그릴 때 무엇을 기다리는지 아직 안 쟀다.
