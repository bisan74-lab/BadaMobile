...
| 3 | 강제 업데이트 게이트(`app_gate.json` 다운로드) | **5초**(타임아웃) |
| 4 | 조석 조회 | 왕복 1회 |
...
- **3번이 지금 가장 크다.** `BadaMobileApp.build`가 `appGateProvider`를
  `when(loading: 스피너)`로 받아서, 게이트 응답이 올 때까지 **화면 전체가
  스피너 하나**다. 게이트를 기다리는 동안 `TideScreen`이 아예 안 만들어지므로
  **조석 요청조차 나가지 않는다** — 게이트 시간이 데이터 시간에 그대로
  더해진다. 테스트가 이 순서(`게이트 응답 → 조석 요청 → 첫 화면`)를 단언으로
  못박아 뒀고, 고치면 그 단언들을 반대로 뒤집어야 한다.
- 첫 화면이 기다리는 데이터는 **조석 하나뿐**이다. 예보·낚시지수는 "낚시정보"
  패널을 열어야 나가므로 첫 진입 비용이 아니다(패널 기본값이 바뀌면 테스트가
  알려 준다).

측정 방식 주의: 테스트가 찍는 **절대 시간은 프레임 루프 오버헤드가 섞여
부풀려져 있다.** 의미가 있는 것은 순서와 "순차 왕복 몇 번"이다(지금 2번 →
게이트를 빼면 1번).
