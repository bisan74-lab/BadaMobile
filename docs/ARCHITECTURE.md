...
- 화면 구조는 `Stack`이다(스크롤 페이지가 아니다): 지도가 화면 전체를
  ...
  - 지도(`_WindMapArea`, `InteractiveViewer`로 항상 감쌈): 풍속 색상
    ...
    - `MapProjection`/`LatLonBounds`(`widgets/map_projection.dart`): 지도의
      ...
      26.5~45.5°N·118.5~136.5°E)는 대한민국이 정확히 중앙에 오도록 상하·
      좌우 대칭으로 잡았다(축소했을 때도 한국이 화면 중심에 보이도록) —
      그러면서도 중국 동해안·일본까지 주변 맥락이 함께 보인다. 좌표를
      바꾸면 `country_borders_data.dart`(해안선)도 같은 범위로 다시 추출
      해야 한다. 이 범위와 바람장 격자 범위를 **동일하게** 맞춰 지도
      전체에 바람 히트맵이 채워지도록 했다 — Open-Meteo는 위경도만 주면
      전 세계 어디든 응답하므로, 별도의 "글로벌 기상 API" 연동 없이 기존
      리포지토리의 격자 범위(`OpenMeteoWindFieldRepository.minLat` 등)만
      넓히면 된다(현재 10×12=120 격자점).
      확대(`maxScale: 6`)·축소(`minScale: 1`) 모두 고정 상한/하한이 있어
      끝없이 확대/축소되지 않는다.
