...
1. ~~**히트맵 굽는 비용**~~ → **완료**(아래 "개선 1"·"개선 3" 참고).
...
### 개선 3 — 래스터를 여러 아이솔레이트에서 나눠 굽는다 (2026-08)

개선 1로 드래그 중에는 저해상도만 굽게 됐지만, **손을 뗀 뒤 고해상도가
채워지기까지** 실기기에서 약 3초가 걸린다는 제보를 받았다. 픽셀 하나마다
bicubic 샘플 2번 + FBM 노이즈 4번(각 3옥타브 × 4해시)이 도는데 88만 px이니
한 아이솔레이트로는 감당이 안 된다.

픽셀끼리 서로를 참조하지 않으므로 **가로 띠로 쪼개 동시에 굽는다**
(`_fillInBands`). 띠 수는 래스터 크기와 기기 코어 수로 정한다(`_bandCount`) —
아이솔레이트를 띄우는 비용이 있어 작은 래스터는 오히려 손해다.

| | 이전 | 지금 (4코어) |
|---|---|---|
| 배경 420×404 (17만 px) | 366 ms | **220 ms** |
| 핵심영역 1100×800 (88만 px) | 1507 ms | **639 ms** |

**띠마다 자기 높이로 위경도를 계산하면 안 된다** — 경계에서 색이 끊긴다.
각 띠는 전체 높이 기준의 `yStart`~`yEnd`만 받고 위경도는 전체 높이로 잡는다.

여기서 더 줄이려면 남은 손잡이는 **핵심영역 텍스처 크기**다(선명도 ↔ 속도).
1100×800을 900×650으로 줄이면 픽셀이 34% 줄지만 확대했을 때 더 뭉개진다.
