...
버벅인다.** 아직 손대지 않았고, 아래는 코드에서 확인한 구체적 후보다.
고칠 때는 **먼저 측정하고**(영상 프레임 분석이나 `Timeline`), 고친 뒤 같은
방법으로 다시 재는 순서를 지킨다 — 예전에 측정 없이 캐싱을 의심했다가 틀린
진단으로 한 번 헛돌았다.

1. **지도에서 지점을 탭했을 때 상세 예보가 늦게 뜬다 (1~2초)**
   `OpenMeteoMarineRepository._fetchAt`이 지점 하나당 **HTTP 5건**을 병렬로
   던진다(GFS 파랑 · WAM 총파고 · WAM 너울 · 수온 · 육상예보). 홈·낚시정보는
   이미 서버 파일(`github_point_forecast_repository`)로 옮겼지만, **상세
   예보는 너울·파력이 필요해 직접 호출을 유지**하고 있다.
   → 후보: 결과를 지점·시각 단위로 캐시, WAM 2건을 한 요청으로 합치기,
   먼저 온 GFS로 화면을 그리고 WAM은 도착하는 대로 덮기(점진적 표시).

2. **바람지도 시간 이동 시 래스터를 매번 다시 굽는다**
   `_rebuildHeatmap`이 시간 스텝마다 전체 bbox(420×404)와 핵심영역
   (1100×800 = 88만 px)을 새로 계산한다. 아이솔레이트로 옮기고 두 장을 한 번에
   교체하는 것까지는 했지만(`_HeatmapPair`), **굽는 비용 자체는 그대로**다.
   → 후보: 앞뒤 시각을 미리 구워 두기(prefetch), 스크럽 중에는 저해상도만
   쓰고 손을 뗐을 때 고해상도로 올리기, 핵심영역 텍스처 크기 재검토.

3. **파티클 레이어가 매 프레임 다시 그린다**
   `WindMapPainter.shouldRepaint`가 무조건 `true`다(애니메이션이라 의도된
   것이지만, 지도 위 다른 레이어까지 함께 무효화되지 않는지 확인이 필요).
   → 후보: 파티클만 `RepaintBoundary`로 분리(단, 해안선·라벨은 감싸면 안 된다
   — 확대 시 흐려진다).

4. **해안선 경로를 매번 새로 만든다**
   `country_borders_data.dart`가 **약 525KB**의 좌표 배열이고,
   `CoastlinePainter`는 `projection`이나 `scale`이 바뀔 때마다 `Path`를 다시
   만든다. 확대·팬 중에 이 비용이 반복된다.
   → 후보: 배율 구간별 `Path` 캐시, 화면 밖 폴리라인 스킵.

5. **탭 전환 시 첫 프레임이 늦다**
   `AppShell`이 `IndexedStack`으로 세 탭을 모두 살려 두므로 전환 자체는
   싸지만, 각 화면이 처음 그려질 때 provider가 한꺼번에 깨어난다.
   → 후보: 첫 진입 시 무엇을 기다리는지 측정부터.
