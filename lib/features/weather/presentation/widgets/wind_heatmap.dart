...
// 파랑 계열은 저속(0~7)에 스톱을 촘촘히 둬 레벨이 더 세분화되고(연보라→파랑→
// 청록→시안→틸), 강풍 제트(동해 등, 약 11~19)는 초록에서 곧장 **밝은 노랑→
// 금색→주황→빨강**으로 넘어가 Windy처럼 노랑이 뚜렷하게 나온다. 예전엔 이
// 구간이 칙칙한 올리브/갈색이라 동해가 초록으로만 보였다.
// 파랑 계열은 저속(0~7)에 스톱을 촘촘히 둬 레벨이 세분화되고(연보라→파랑→
// 청록→시안→틸), 초록은 ~11까지 유지한다. 노랑→금색→주황→빨강은 **강풍
// 제트(약 13~21)에서만** 나오게 위로 밀었다 — 예전엔 노랑이 11~13부터 나와
// 바다 곳곳이 노래서 과했다(사용자 피드백). 이제 동해 제트 중심만 노랗고
// 주변은 초록/연두로 자연스럽다.
