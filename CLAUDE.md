...
- **좁은 세로 칩에서 라벨을 키우는 건 답이 아니다.** 배율 상한만 걸었더니
  폭 52px 안에서 "낚시정/보"처럼 2~3줄로 쪼개져 오히려 읽기 나빠지고 메뉴가
  본문을 덮었다(2026-08-06 제보). Material 표준(`NavigationRail.labelType`,
  `NavigationBar.labelBehavior`)대로 **공간이 부족하면 라벨을 뗀다** —
  `navChipLabelMaxScale`(1.2)를 넘으면 아이콘만 남기고 아이콘을 1.7배까지
  키운다. 뜻이 사라지지 않게 `Semantics` 라벨과 `Tooltip`을 남긴다.
