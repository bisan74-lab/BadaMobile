...
- **쭈꾸미·갑오징어·문어는 관측이 아니라 추정이다**
  ...
  - **제철 가중치(`_season`)는 12개월을 빠짐없이 채운다.** 빠진 달이 하한으로
    ...
    쭈꾸미·갑오징어는 **9월(금어기 해제)·10월이 최고, 11월부터 급감**
    (사용자 실사용 경험).
  - **"매우나쁨"은 날씨가 나쁠 때만 나온다.** 조류 선호 곡선에 하한
    `_tideFloor`(0.30)를 둬, 조류가 아무리 벗어나도 등급을 몇 칸 낮출 뿐
    바닥까지 끌어내리지 못하게 했다 — 제철엔 물이 많이 흘러도 잡히기
    때문이다. 바닥(매우나쁨, 0.08 미만)은 강풍·높은 파고만 닿을 수 있다.
