...
- **바람장 격자 총 좌표 수는 600 미만 유지**(현재 21×24=504): Open-Meteo
  무료 한도가 분당 600콜이고 다지점 요청은 좌표 1개=1콜이라, 넘기면 요청
  한 번에 한도를 초과해 **매번 429 → 지도가 항상 합성 폴백**이 된다(837점
  으로 올렸다가 실제로 겪음). 색 세밀함이 부족하면 격자 대신 히트맵 래스터
  해상도나 보간을 손본다.
