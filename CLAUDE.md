...
`clampAppTextScale`을 `MaterialApp.builder`에 넘긴다). 물때표·상세예보 표·
설정처럼 한 화면에 값을 촘촘히 담는 화면이 많아, 이보다 오르면 글자가 겹치고
잘려 오히려 못 읽는다(2026-08-06 제보 3종). **화면마다 쫓아다니지 말고 여기서
막는다** — 메뉴 하나만 세 번 고쳤는데 같은 종류가 앱 곳곳에 있었다.
사용자 접근성 설정을 일부 무시하는 대가가 있으니 값을 함부로 낮추지 말 것.
**테스트 하네스도 같은 `clampAppTextScale`을 써야** 실제와 같은 조건이 된다.

**레이아웃을 건드리면 `text_scale_layout_test.dart`를 함께 돌린다** — 시스템
글자 크기 1.0~2.0배 × 화면 크기 3종으로 네 화면을 그려 오버플로를 잡는다
(상한 덕분에 실제 적용 배율은 최대 1.5다).
`size.height - N` 같은 뺄셈은 **항상 clamp**할 것(음수가 되면 오버플로가
아니라 화면이 통째로 깨진다). 본문 화면에 `withClampedTextScaling`을 쓰는 건
접근성 설정 무시라 금지 — 크기가 외부 규격에 묶인 광고 자리와 세로 메뉴
칩(`NavChip`, 폭 52px 고정)만 예외다.

**오버플로 예외를 안 던지는 깨짐도 있다.** 겹침과 "요소가 사라짐"은
`takeException()`에 안 잡히므로 따로 확인한다(같은 파일의 뒤쪽 두 테스트).
- **오른쪽 세로 메뉴는 두 벌**이다 — 물때 화면 미니 메뉴와 앱 탭 레일. 둘 다
  `NavChip`을 쓰고, 미니 메뉴는 **화면 최상위 스택**에서 `88 +
  sideNavRailHeight(context)` 위에 놓아 레일 바로 위에 붙인다. 레일 높이를
  **상수로 박으면 안 된다**(배율이 오르면 레일만 길어져 겹친다). 높이를 재는
  `navChipsHeight`는 칩이 실제로 쓰는 `DefaultTextStyle`로 재야 한다 —
  fontSize만 준 맨 TextStyle로 재면 20%쯤 낮게 나와 그만큼 겹친다.
- **칩 라벨을 떼는 건 답이 아니었다.** 좁은 칸에서 글자가 쪼개지길래
  Material 표준(`NavigationRail.labelType`)대로 아이콘만 남겨 봤더니
  "아이콘만으로는 알아보기 힘들다"는 제보를 받았다. 지금은 라벨을 그대로 두되
  **칩에만 배율 상한 `navChipMaxScale`(1.3)을 한 번 더 건다** — 본문은 앱 상한
  1.5로 시원하게 두고 메뉴만 눌러 두는 것이다(사용자 요구). 라벨·아이콘·높이
  계산이 모두 같은 `_chipScaler`를 써야 어긋나지 않는다.
  세 화면(물때·바람지도·앱 셸)의 레일이 **모두 같은 `NavChip`**을 쓴다 —
  바람지도만 자기 방식으로 그리던 시절엔 거기만 "물때날/씨"로 쪼개졌다.
- **달력 날짜 칸 비율도 고정하면 안 된다**(`mul_ttae_calendar.dart`):
  한 칸에 날짜와 물때 이름 두 줄이 들어가서, 비율을 고정하면 큰 글자에서
  글자가 칸 밖으로 밀려 아랫줄 날짜와 겹친다. `_cellAspectRatio`가 배율만큼
  칸을 세로로 늘리고, 두 줄 다 `FittedBox`+`maxLines: 1`로 칸 안에 가둔다.
- **검사 화면 크기를 줄이지 말 것.** 겹침은 특정 조합에서만 난다 —
  2026-08-06 제보 건은 360×780 · 1.6배에서만 재현됐고, 목록에 그 크기가 없어
  검사를 통과하고 있었다.
...
- **지도 히트맵은 래스터 두 장을 한 벌(`_HeatmapPair`)로 묶어 한 번에
  ...
  **래스터는 가로 띠로 쪼개 여러 아이솔레이트에서 동시에 굽는다**
  (`_fillInBands`, 4코어 기준 핵심영역 1507→639ms). 띠마다 자기 높이로
  위경도를 계산하면 경계에서 색이 끊기므로, 각 띠는 전체 높이 기준의
  `yStart`~`yEnd`만 받는다.
...
- **낚시지수도 서버 파일 우선**(`features/fishing/.../github_fishing_repository.dart`):
  ...
  **이 API의 `numOfRows` 상한은 300이다** — 넘기면 HTTP 200에
  ...
  실제 응답을 먼저 확인한다.
  **낡은 파일은 앱이 뒤로 미룬다** — 오늘 날짜가 없거나 `generated`가 30시간
  넘게 지난 파일이면 직접 호출을 **먼저** 시도한다(수집이 멈춘 상태로 본다).
  단 직접 호출까지 실패하면 그 낡은 파일로 되돌아온다 — 이틀 전 것이어도
  실제 관측 기반 값이 합성 데이터보다는 낫다.
  **"오늘 날짜가 들어 있는가"만 보면 안 된다**: 이 API는 며칠 앞까지 주므로
  수집이 멈춘 뒤에도 이틀쯤은 오늘이 들어 있는 낡은 파일이 남는다. 같은
  이유로 워크플로의 건너뛰기 판단(`--check-published`)도 **`generated`
  나이**로 한다(2026-08-06에 날짜 기준으로 봤다가 수집을 통째로 건너뛰었다).
  ...
  크론 + **이미 최근 파일이 올라와 있으면 수집 자체를 건너뜀**
  (`--check-published`).
  **data.go.kr은 GitHub 러너에 간헐적으로 응답하지 않는다.** 2026-08-04~06에
  네 번 연속 20초 무응답으로 실패했다가, 같은 커밋 같은 요청이 몇 시간 뒤엔
  8초에 성공했다. 러너는 실행마다 다른 IP를 받으므로 어떤 IP를 뽑느냐에 따라
  갈리는 것으로 보인다. **연속 실패를 봐도 "차단됐다"고 단정하지 말 것** —
  `probe-fishing.yml`의 **diagnose 모드**(`tool/probe_datagokr.py`)로 러너에서
  DNS→TCP→TLS→HTTP를 층별로 재고, 키 지문(길이·SHA-256 앞 12자, 값은 안 찍음)
  까지 확인한 뒤 판단한다. 대응은 크론 하루 4회다 — 한 번 막혀도 다른 시각에
  다른 IP로 다시 시도하면 뚫린다.
- **조석은 "당일 극값이 있는가"까지 확인해야 성공이다**
  (`data_go_kr_tide_repository.dart` / `..._obs_repository.dart`): 전날·당일·
  다음날 3일치를 받는데, 예전엔 **3일 전체가 빈 경우만** 실패로 쳤다. 그래서
  전날·다음날은 오고 **당일만 빠지면**(간헐적 응답 실패) 그대로 성공으로
  올라가, 화면에 만조·간조 카드가 하나도 없고 조위 곡선이 마지막 극값으로
  25시간 내내 고정돼 **조류세기가 0%**로 떴다(2026-08-06 사용자 제보).
  당일 극값이 비면 던져야 폴백 체인(고저조 → 합성)이 이어진다.
