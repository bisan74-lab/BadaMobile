...
- **낚시지수도 서버 파일 우선**(`features/fishing/.../github_fishing_repository.dart`):
  ...
  **"오늘 날짜가 들어 있는가"만 보면 안 된다**: 이 API는 며칠 앞까지 주므로
  ...
  (`--check-published`).
  **2026-08 현재 이 수집은 GitHub에서 실패한다 — 우리 코드 문제가 아니다.**
  data.go.kr이 8/4 무렵부터 GitHub Actions 대역(미국)에 응답을 주지 않는다.
  같은 요청이 한국 IP + 정상 키로는 0.26초에 200이다(실측으로 확인: 파라미터·
  키·재시도·UA 모두 무죄, 차이는 출발 IP뿐). **고치려면 수집을 한국 IP에서
  돌려야 한다.** 그동안 앱은 낡은 파일을 보고 직접 호출로 넘어가므로 정확도는
  유지되고 지역 변경만 몇 초 느려진다. 크론은 하루 1회로 줄여 뒀고(알림 소음),
  차단이 풀리면 주석 처리한 세 줄을 되살린다.
