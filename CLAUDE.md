...
**앱 전체에 글자 배율 상한 `kMaxTextScale`(1.3)이 걸려 있다**(`app/app.dart`의
...
- **칩 라벨을 떼는 건 답이 아니었다.** 좁은 칸에서 글자가 쪼개지길래
  ...
  "아이콘만으로는 알아보기 힘들다"는 제보를 받았다. 지금은 **앱 전체 배율
  상한**(`kMaxTextScale`)으로 쪼개짐 자체를 없애고 라벨을 그대로 둔다.
